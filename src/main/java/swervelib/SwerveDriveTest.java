...
public class SwerveDriveTest {
   ...
   */
  public static double findDriveMotorKV(
      SwerveDrive swerveDrive, double minMovement, double testDelaySeconds, double maxVolts) {
  ...
  public static double findCouplingRatio(SwerveDrive swerveDrive, double volts, boolean automatic) {
    ...
    for (SwerveModule module : swerveDrive.getModules()) {
      ...
      Timer.delay(1);
      Rotation2d startingAbsoluteEncoderPosition =
          Rotation2d.fromDegrees(absoluteEncoder.getAbsolutePosition());
      double driveEncoderPositionRotations =
      ...
      if (automatic) {
        ...
        while (!Rotation2d.fromDegrees(absoluteEncoder.getAbsolutePosition())
        ...
        DriverStation.reportWarning(
            "Spin the "
                ...
                + " module 360 degrees now, you have 1 minute.\n",
            false);
        Timer.delay(60);
      }
      ...
      DriverStation.reportWarning(
          module.configuration.name + " Coupling Ratio: " + couplingRatio, false);
  ...
  /**
   * Logs output, position and velocuty info form the drive motor to the SysIdRoutineLog <br>
   * Although SysIdRoutine expects to be logging Voltage, this function logs in Duty-Cycle (percent
   * output) because it results in correctly adjusted values in the analysis for use in this
   * library.
   *
  ...
  public static void logDriveMotorDutyCycle(SwerveModule module, SysIdRoutineLog log) {
    logDriveMotorActivity(
        module,
        log,
        () -> module.getDriveMotor().getVoltage() / RobotController.getBatteryVoltage());
  }
   ...
   */
  public static void logDriveMotorActivity(
      SwerveModule module, SysIdRoutineLog log, Supplier<Double> powerSupplied) {
   ...
   */
  public static SysIdRoutine setDriveSysIdRoutine(
  ...
  /**
   * Logs info about the angle motor to the SysIdRoutineLog. <br>
   * Although SysIdRoutine expects to be logging Voltage, this function logs in Duty-Cycle (percent
   * output) because it results in correctly adjusted values in the analysis for use in this
   * library.
   *
   ...
   * @param powerSupplied - a functional supplier of the power to be logged
   */
  public static void logAngularMotorActivity(
      SwerveModule module, SysIdRoutineLog log, Supplier<Double> powerSupplied) {
    double power = powerSupplied.get();
   ...
   */
  public static SysIdRoutine setAngleSysIdRoutine(
      Config config, SubsystemBase swerveSubsystem, SwerveDrive swerveDrive) {
    return new SysIdRoutine(
        config,
        new SysIdRoutine.Mechanism(
            (Voltage voltage) -> {
  ...
  /**
   * Creates a command that can be mapped to a button or other trigger. Delays can be set to
   * customize the length of each part of the SysId Routine
   *
   ...
   */
  public static Command generateSysIdCommand(
      SysIdRoutine sysIdRoutine, double delay, double quasiTimeout, double dynamicTimeout) {
    return sysIdRoutine
        .quasistatic(SysIdRoutine.Direction.kForward)
        .withTimeout(quasiTimeout)
        .andThen(Commands.waitSeconds(delay))
        .andThen(
            sysIdRoutine.quasistatic(SysIdRoutine.Direction.kReverse).withTimeout(quasiTimeout))
        .andThen(Commands.waitSeconds(delay))
