...
public class SwerveMath {
   ...
   * IN METERS) / (GEAR RATIO * ENCODER RESOLUTION)
   *
   ...
   * @param maxSpeed Maximum velocity in meters per second to use for the feed forward, should be as
   *     close to physical max as possible.
   * @param wheelGripCoefficientOfFriction Wheel grip coefficient of friction for kA
   *     (voltage/(cof*9.81))
   * @return Drive feedforward for drive motor on a swerve module.
   */
  public static SimpleMotorFeedforward createDriveFeedforward(
      double optimalVoltage, double maxSpeed, double wheelGripCoefficientOfFriction) {
  ...
  public static void antiJitter(
      SwerveModuleState moduleState, SwerveModuleState lastModuleState, double maxSpeed) {
