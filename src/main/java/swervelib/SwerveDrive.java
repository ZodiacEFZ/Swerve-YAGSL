...
public class SwerveDrive {
   ...
   * @param maxSpeedMPS Maximum speed of the robot in meters per second, remember to use {@link
  ...
  public SwerveDrive(
    ...
    if (SwerveDriveTelemetry.isSimulation) {
      ...
      for (int i = 0; i < swerveModules.length; i++) {
        this.swerveModules[i].configureModuleSimulation(
            mapleSimDrive.getModules()[i], config.physicalCharacteristics);
      }
  ...
  public void drive(
    ...
    SwerveModuleState[] swerveModuleStates =
        kinematics.toSwerveModuleStates(robotRelativeVelocity, centerOfRotationMeters);
   ...
   * allow for usage of {@link SwerveDriveKinematics#desaturateWheelSpeeds(SwerveModuleState[]
   * moduleStates, ChassisSpeeds desiredChassisSpeed, double
   * attainableMaxModuleSpeedMetersPerSecond, double attainableMaxTranslationalSpeedMetersPerSecond,
   * double attainableMaxRotationalVelocityRadiansPerSecond)}
   *
   * @param desiredStates A list of SwerveModuleStates to send to the modules.
   * @param isOpenLoop Whether to use closed-loop velocity control. Set to true to disable
  ...
  public void drive(
    ...
    for (SwerveModule module : swerveModules) {
      ...
      double wheelRadiusMeters =
          Units.inchesToMeters(module.configuration.conversionFactors.drive.diameter) / 2;
      ...
      double feedforwardVoltage =
          driveMotorModel.getVoltage(
              ...
              feedforwardForces[module.moduleNumber].in(Newtons)
                  * wheelRadiusMeters
                  / driveGearRatio,
  ...
  public void setChassisSpeeds(ChassisSpeeds robotRelativeSpeeds) {
    ...
    setRawModuleStates(
        kinematics.toSwerveModuleStates(robotRelativeSpeeds), robotRelativeSpeeds, false);
  }
  ...
  public Optional<Pose2d> getSimulationDriveTrainPose() {
    odometryLock.lock();
    Optional<Pose2d> simulationPose =
        getMapleSimDrive().map(AbstractDriveTrainSimulation::getSimulatedDriveTrainPose);
    odometryLock.unlock();
   ...
   * axis or angles, and apply {@link SlewRateLimiter} to given inputs.
  ...
  /**
   * Enable or disable the {@link SwerveModuleConfiguration#useCosineCompensator}
   ...
   */
  public SwerveModuleState[] toServeModuleStates(
      ChassisSpeeds robotRelativeVelocity, boolean optimize) {
    if (optimize) {
      robotRelativeVelocity =
          movementOptimizations(
              robotRelativeVelocity, chassisVelocityCorrection, angularVelocityCorrection);
