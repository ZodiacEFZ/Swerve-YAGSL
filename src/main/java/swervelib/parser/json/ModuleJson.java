...
/** {@link SwerveModule} JSON parsed class. Used to access the JSON data. */
  ...
  public SwerveModuleConfiguration createModuleConfiguration(
    ...
    if (!conversionFactors.works() && physicalCharacteristics.conversionFactor == null) {
      throw new RuntimeException(
          "No Conversion Factor configured! Please create SwerveDrive using \n"
    ...
    // If both are defined, override 0 with the physical characterstics input.
    {
      ...
      conversionFactors.drive =
          conversionFactors.isDriveEmpty()
              ? physicalCharacteristics.conversionFactor.drive
              : conversionFactors.drive;
    }
    ...
    if (conversionFactors.angle.factor == 360
        && absEncoder != null
        && (absEncoder instanceof SparkMaxEncoderSwerve
            && angleMotor.getMotor() instanceof SparkMax)) {
      angleMotor.setAbsoluteEncoder(absEncoder);
    } else if ((absEncoder instanceof ThriftyNovaEncoderSwerve
